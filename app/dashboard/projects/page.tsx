...
const AIChatPanel = dynamic(() => import("@/components/ai/AIChatPanel").then((m) => ({ default: m.AIChatPanel })), { ssr: false });

function ProjectsPageInner() {
  const router = useRouter();
  const view = (useSearchParams().get("view") ?? "projects") as TabValue;
  const { data: rawProjects = [], isLoading, isError } = useErpList("Project", {
    fields: ["name", "status", "percent_complete", "expected_end_date"],
  });
  const stats = useMemo(() => deriveProjectStats((rawProjects as Record<string, unknown>[]).map(mapErpProject)), [rawProjects]);
  ...
  const header = (
    <div className="flex items-center justify-between">
      ...
      <Button variant="primary" onClick={() => router.push("/dashboard/projects/new")}>+ {ctaLabel}</Button>
    </div>
  );
  ...
  if (isError) return (
    <div className="space-y-6">
      {header}
      <Card><CardContent className="flex flex-col items-center justify-center gap-4 py-16 text-center">
    ...
    <div className="space-y-6">
      ...
      {view === "projects" && (
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-4">
          {[0, 1, 2, 3].map((i) => <div key={i} className="min-h-[88px] rounded-xl border border-border bg-card p-6 animate-pulse" />)}
        </div>
      )}
      <Card><CardContent className="p-0"><SkeletonTable rows={8} columns={view === "timesheets" ? 6 : 7} /></CardContent></Card>
    </div>
  );
    ...
    <div className="space-y-6">
      ...
      <Tabs value={view} onValueChange={handleTabChange}>
        <TabsList>
          ...
          <TabsTrigger value="timesheets"><Clock className="h-4 w-4 mr-1.5" />Timesheets</TabsTrigger>
        </TabsList>
