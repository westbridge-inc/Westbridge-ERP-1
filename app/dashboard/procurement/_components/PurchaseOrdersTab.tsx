...
export function PurchaseOrdersTab() {
  ...
  const columns = useMemo(() => getPoColumns(setDeleteTarget), []);

  const handleDelete = useCallback(async () => {
    if (!deleteTarget) return;
    setDeleting(true);
    try {
      await api.erp.delete("Purchase Order", deleteTarget.id);
    ...
    <>
      {!loading && (
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <MetricCard label="Total POs" value={metrics.total} />
          <MetricCard label="Pending" value={metrics.pending} subtextVariant="muted" />
          <MetricCard label="Received" value={metrics.received} subtextVariant="success" />
